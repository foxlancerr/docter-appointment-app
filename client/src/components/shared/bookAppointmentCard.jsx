...
const AppointmentCard = ({ fees, daysAvailable, _id }) => {
  ...
  return (
    <Card className="max-w-md mx-auto bg-white shadow-lg rounded-lg border border-gray-200">
      <CardHeader>
        <CardTitle className="text-2xl text-center font-semibold text-[#023e7d]">
          Let's Discuss
        </CardTitle>
      </CardHeader>
      <CardContent className="p-6 flex gap-5 flex-col">
        <div className="flex items-center border-b-2 border-b-gray-100 pb-2 justify-between">
          <p className="text-[#023e7d]">Fees:</p>
          <p className=" font-semibold text-sm">Rs. {fees}</p>
        </div>
        <div className="flex items-center justify-between border-b-gray-100 pb-2 border-b-2 ">
          <p className="text-[#023e7d]">Days:</p>
        ...
        <div className="flex flex-col">
          {daysAvailable?.map((day, index) => {
            return (
              <div className="flex justify-between items-center" key={day + index}>
                <h4 className="text-lg font-semibold text-[#023e7d]">{day}</h4>
                <p className="text-sm">07:30 PM - 09:00 PM</p>
              </div>
            );
          })}
        </div>
        <div className="flex gap-3 flex-col">
          ...
          <Button
            className="w-full bg-[#023e7d] text-white hover:bg-[#023e7d]/90 flex items-center justify-center"
